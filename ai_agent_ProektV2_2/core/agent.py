...
class AIAgent:
    ...
    def submit_request(self, request: Dict[str, Any], image_path: str = None) -> None:
        """
        Асинхронно отправляет запрос: подготавливает параметры (кодирует изображение, если есть),
        кладёт запрос в очередь для обработки воркером.
        Не возвращает ответ — ответ приходит отдельно через get_response.
        """
        import copy
